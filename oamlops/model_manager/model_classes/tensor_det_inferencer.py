...
ZOOM_LEVEL = {0: 10240, 1: 5120, 2: 1024}

class TensorDetInferencer(BaseInferencer):
    ...
    def __call__(
        ...
        ind_batches, img_batches = batch_samples
        for inds, imgs in zip(ind_batches, img_batches):
            imgs = imgs.to(self.device)
            _, c, h, w = imgs.shape
            batch_data_samples = [DetDataSample(metainfo=dict(img_id=img_id.item(), img_shape=(h, w, c),
                                                                scale_factor=(w/ZOOM_LEVEL[zoom_lvl], w/ZOOM_LEVEL[zoom_lvl]))) for img_id in inds] 
            with torch.no_grad():
                preds = self.model(imgs, batch_data_samples, mode='predict')
    ...
    def pred2dict(self,
        ...
        result['img_id'] = data_sample.metainfo['img_id']
        result['shape'] = data_sample.metainfo['img_shape']
