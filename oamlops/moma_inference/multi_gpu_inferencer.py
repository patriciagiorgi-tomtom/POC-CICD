...
from oamlops.model_manager.model_classes.tensor_det_inferencer import (
    TensorDetInferencer,
)
from oamlops.model_manager.model_registry import get_path_to_the_final_checkpoint
...
def moma_inference_multi_gpu(
    ...
    # set device
    device = (
        "cpu" if "cpu" in job_parameters.device.lower() else f"cuda:{hvd.local_rank()}"
    )
    if "cuda" in device:
    ...
    moma_inferencer = (
        TensorDetInferencer
        if job_parameters.inferencer == "TensorDetInferencer"
        else DetInferencer
    )
