...
class InferenceJobParameters:
    ...
    bbox_classification_rescale_size = (80, 80)
    classification_data_output_path: str | None = None
    bbox_resize: bool = True
    save_bbox: bool = False
    bbox_crop_storage_path : str | None = None
    mount_source_point: str | None = None
    mount_destination_point: str | None = None
    save_bbox_zone: str | None = None
    

    def __post_init__(self):
        if self.inferencer == "ImageClassificationInferencer":
            if self.save_bbox:
                self.mount_source_point = os.path.join(self.mount_source_point, self.save_bbox_zone)
            return
...
class JobParametersProcessor:
    ...
    def get_checkpoint_run_id(
        self,
        task_key: str = "train_mmdetection_model",
        parameter_key: str = "mlflow_run_id",
    ) -> str:
    ...
    def set_ci_cd_pipeline(
        self,
        task_key: str = "train_mmdetection_model",
        parameter_key: str = "ci_cd_pipeline",
    ) -> str:
        try:
            ci_cd_pipeline = ast.literal_eval(
                dbutils.jobs.taskValues.get(taskKey=task_key, key=parameter_key)
            )
        except:
