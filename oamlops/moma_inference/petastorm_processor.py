...
def transform_row(pd_batch: pd.DataFrame, image_shape: tuple, resize: bool = False):
    pd_batch["image"] = pd_batch["panorama_img"].map(lambda x: transform_image(x, image_shape, resize))
    pd_batch = pd_batch.filter(["frame_index", "image"])
    return pd_batch


def get_transform_spec(image_shape: tuple = (2666, 1333), resize: bool = False):
    ...
    return TransformSpec(
        partial(transform_row, image_shape=image_shape, resize=resize),
        edit_fields=[
            ("image", np.float32, (image_shape[1], image_shape[0], 3), False),
