...
def create_prediction_df(
    ...
    flattened_predictions = [
        prediction
        for batch_predictions in image_detections
        for prediction in batch_predictions["predictions"]
    ]
    flattened_indices = [
        {"frame_index": frame_index}
        for batch_frames in frame_indices
    ...
    prediction_dict = [
        {**detections, **indices} for detections, indices in prediction_data
    ]

    # Create raw spark dataframe from dict
    predictions_df = spark.createDataFrame(prediction_dict)
    predictions_df = predictions_df.withColumn(
        "join_index", F.monotonically_increasing_id()
    )

    score_df = predictions_df.select(
        "join_index", F.posexplode("scores").alias("pos", "score")
    )
    label_df = predictions_df.select(
        "join_index", F.posexplode("labels").alias("pos", "class")
    )
    bbox_df = predictions_df.select(
        "join_index", F.posexplode("bboxes").alias("pos", "bbox")
    )

    pred_joined_df = score_df.join(label_df, on=["join_index", "pos"]).join(
        bbox_df, on=["join_index", "pos"]
    )
    ...
    predictions_df = predictions_df.filter(
        F.col("score") > job_params.confidence_threshold
    ).drop("pos", "join_index")
    ...
    rescale_width_ratio = (
        job_params.original_image_size[0] / job_params.rescale_image_size[0]
    )
    rescale_height_ratio = (
        job_params.original_image_size[1] / job_params.rescale_image_size[1]
    )
    if job_params.rescale_bbox_by_zoom_level:
        rescale_width_ratio *= (
            job_params.panorama_zoom_size[0] / job_params.original_image_size[0]
        )
        rescale_height_ratio *= (
            job_params.panorama_zoom_size[1] / job_params.original_image_size[1]
        )

    predictions_df = (
        predictions_df.withColumn(
            "bbox_x", (F.col("bbox")[0] + F.col("bbox")[2]) * rescale_width_ratio / 2
        )
        .withColumn(
            "bbox_y", (F.col("bbox")[1] + F.col("bbox")[3]) * rescale_height_ratio / 2
        )
        .withColumn(
            "bbox_w", (F.col("bbox")[2] - F.col("bbox")[0]) * rescale_width_ratio
        )
        .withColumn(
            "bbox_h", (F.col("bbox")[3] - F.col("bbox")[1]) * rescale_height_ratio
        )
        .drop("bbox")
    )
